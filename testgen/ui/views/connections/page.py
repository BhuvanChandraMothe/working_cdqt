...
# from testgen.ui.components import widgets as testgen  # Keeping import for potential use
...
#             form_kwargs = connection or {"sql_flavor": sql_flavor, "connection_id": connection_id,
...
#             # )
#             data = form.model_dump() # Getting data directly from the form
#             data.update({
...
#             renderer_func = print if connection_status.successful else print
#             renderer_func(connection_status.message)
...
#                 print(connection_status.details)

#             connection_status = None
#         else:
#             # This is needed to fix a strange bug in Streamlit ...
#             time.sleep(0.1)
...
#                     data["private_key"] = data["private_key"].decode("utf-8") if isinstance(data["private_key"], bytes) else data["private_key"]

#                 connection_service.edit_connection(data)
#                 print("Changes have been saved successfully.")
#                 time.sleep(1)
#                 # st.rerun() # Removed Streamlit rerun
...
#         will_run_profiling = True # Defaulting to True as streamlit state is removed
#         # testgen.wizard(
...
#         # )
#         print("Data Configuration Setup initiated")
#         step_0_data, step_0_valid = self.create_table_group_step(project_code, connection)
#         if step_0_valid and step_0_data:
#             run_profiling, step_1_valid = self.run_data_profiling_step(step_0=testgen.WizardStep(results=step_0_data))
#             if step_1_valid:
#                 self.execute_setup(None, testgen.WizardStep(results=step_0_data), testgen.WizardStep(results=run_profiling))
#         else:
#             print("Table group creation step was not valid.")
...
#                 # ]))
#                 LOG.warning(f"Table group form validation error: {error}")
#                 is_valid = False
#         except Exception:
#             LOG.exception("unexpected form validation error")
#             # st.error("Unexpected error displaying the form. Try again")
...
#             print("A table group is required to complete this step.")
#             return False, False
...
#         print(f"Execute profiling for the table group **{table_group.get('table_groups_name', 'unknown')}**? {'Yes' if run_profiling else 'No'}. _{profiling_message}_")

#         return run_profiling, True

#     def execute_setup(
#         self,
#         container, # Removed DeltaGenerator type hint
#         step_0: testgen.WizardStep[dict],
#         step_1: testgen.WizardStep[bool],
#     ) -> bool:
#         table_group = step_0.results
#         table_group_name: str = table_group.get("table_groups_name", "default_group")
#         should_run_profiling: bool = step_1.results

#         # with container.container():
#         #     status_container = st.empty()
#         status_container_print = print

#         try:
#             status_container_print(f"Creating table group **{table_group_name.strip()}**.")
#             table_group_id = table_group_service.add(table_group)
#             TableGroupForm.construct().reset_cache()
#         except Exception as err:
#             status_container_print(f"Error creating table group: {err!s}.")

#         if should_run_profiling:
#             try:
#                 status_container_print("Starting profiling run ...")
#                 run_profiling_in_background(table_group_id)
#                 status_container_print(f"Profiling run started for table group **{table_group_name.strip()}**.")
#             except Exception as err:
#                 status_container_print(f"Profiling run encountered errors: {err!s}.")
...
#             #         on_click=lambda: (
#             #             st.session_state.__setattr__("setup_data_config:navigate-to", "profiling-
...
class ConnectionsPage:
    ...
    def render(self, project_code: str, **_kwargs) -> None:
        ...
        user_can_edit = user_session_service.user_can_edit()
        if not has_table_groups and user_can_edit:
            self.setup_data_configuration(project_code, connection.to_dict())
    ...
    def show_connection_form(self, selected_connection: dict, _mode: str, project_code) -> None:
        ...
        if user_session_service.user_is_admin():
            pass  # Removed Streamlit button logic
        ...
        if is_submitted():
            if not data.get("password") and not data.get("connect_by_key"):
                ...
                if data.get("private_key"):
                    data["private_key"] = data["private_key"].decode("utf-8") if isinstance(data["private_key"],
                                                                                             bytes) else data[
                        "private_key"]
    ...
    def setup_data_configuration(self, project_code: str, connection: dict) -> None:
        ...
        step_0_data, step_0_valid = self.create_table_group_step(project_code, connection)
        if step_0_valid and step_0_data:
            run_profiling, step_1_valid = self.run_data_profiling_step(
                step_0=testgen.WizardStep(results=step_0_data))
            if step_1_valid:
                self.execute_setup(None, testgen.WizardStep(results=step_0_data),
                                   testgen.WizardStep(results=run_profiling))
        else:
            print("Table group creation step was not valid.")
    ...
    def run_data_profiling_step(self, step_0: testgen.WizardStep | None = None) -> tuple[bool, bool]:
        ...
        print(
            f"Execute profiling for the table group **{table_group.get('table_groups_name', 'unknown')}**? {'Yes' if run_profiling else 'No'}. _{profiling_message}_")
